...
tone(source, 440, 4);
tone(music, 880, 0.5);
const microphone = path.join(dir, "microphone.wav");
tone(microphone, 1320, 4);
...
const baselines = new Map<string, Float32Array>();
p.cursor = [200, 1500, 2200, 3400].map((time) => ({
  time,
  x: 0.5,
  y: 0.5,
  click: true,
...
for (const clicks of [false, true])
  for (const microphoneEnabled of [false, true])
    for (const muted of [false, true]) {
      p.appearance.clickSound = clicks ? "soft" : "none";
      p.appearance.clickSoundVolume = 0.25;
      p.microphoneAudio = microphoneEnabled
        ? { file: "microphone.wav", volume: 0.2, muted: false }
        : undefined;
      ...
      const out = path.join(
        ...
        (clicks ? "clicks-" : "") +
          ...
          (muted ? "music-only.mp4" : "mixed.mp4"),
      );
      const temp = out + ".pending.mp4";
      const child = spawn(
        ffmpeg,
        exportArgs(p, source, temp, 30, "mp4", music, microphone, "pipe:3"),
        {
      ...
      void encoded.catch(() => {});
      assert.deepEqual(
        clickSoundCues(p).map((cue) => cue.time),
        [200, 1100, 1700],
      );
      const clickPCM = clickAudioChunk(
        clickSoundCues(p),
        clickSoundBank("soft"),
        48000,
        0,
      ...
      clickPipe.on("error", () => {});
      const audio = clicks
        ? pipeline(Readable.from([Buffer.from(clickPCM.buffer)]), clickPipe)
        : Promise.resolve();
      if (!clicks) clickPipe.end();
      ...
      const key = `${microphoneEnabled}-${muted}`;
      if (!clicks) baselines.set(key, samples);
      else {
        const baseline = baselines.get(key)!;
        for (const time of [0.2, 1.1, 1.7]) {
          let energy = 0,
            error = 0;
          const start = Math.round(time * 48000);
          for (let i = start; i < start + 2400; i++) {
            const expected = clickPCM[i];
            energy += expected * expected;
            error += (samples[i] - baseline[i] - expected) ** 2;
          }
          const relativeError = Math.sqrt(error / energy);
          console.log(
            `Click waveform ${time}s mic/mute=${key}: relative error ${relativeError.toFixed(4)}`,
          );
          assert.ok(
            relativeError < 0.6,
            `Click ${time}s ${key}: relative waveform error ${relativeError}`,
          );
        }
      }
      ...
      const observations = [0.25, 1.25].map((t) => ({
        ...
        microphone: amplitude(t, 1320),
      }));
      for (const result of observations) {
        assert.ok(
          Math.abs(result.microphone - (microphoneEnabled ? 0.0125 : 0)) <
            0.002,
          JSON.stringify(result),
        );
        assert.ok(
          Math.abs(result.music - 0.03125) < 0.003,
          JSON.stringify(result),
        );
        assert.ok(
          muted
            ? result.source < 0.002
            : Math.abs(result.source - 0.0625) < 0.006,
          JSON.stringify(result),
        );
      }
      console.log(
        clicks ? "With clicks" : "Without clicks",
        microphoneEnabled ? "With microphone" : "Without microphone",
        muted ? "Source muted" : "Mixed",

...
## Shared editor-clock scheduling

Click preview no longer extrapolates React's throttled time with an independent wall clock. It reads the same time ref as the composition preview. A 40 ms lookahead replaces the previous 150 ms queue. An explicit seek revision clears queued sources even for small forward seeks; backward movement clears scheduling for loops. Seek updates the shared ref immediately. Late transients use an elapsed offset in real seconds at the selected preview speed, and expired transients are skipped.

Three scheduler regressions cover repeated observations of a stalled clock, small forward seeks, backward loop replay, and 2× late/expired samples. All 118 core tests and production compilation pass. This bounds prequeued audio to 40 ms when the editor clock stops; it cannot retract audio already emitted. The editor transport itself still uses animation time rather than decoded-video timestamps, so this does not prove synchronization during media decoder stalls. Actual audible seek/loop verification remains outstanding.

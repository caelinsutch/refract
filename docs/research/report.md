...
The planned implementation is called **Frame Studio**. It aims to reproduce the observed workflows while retaining its own source code, application identity, file format, and assets. No vendor account service, licensing mechanism, or proprietary helper is a runtime dependency.
...
The encoder chunk directly constructs a `VideoEncoder`. A separate graphics-heavy chunk contains rendering and texture-management code. WebCodecs exposes codec interfaces but does not itself require support for a particular codec.[^17] Frame Studio must probe capabilities and handle unsupported encoder configurations rather than assuming every Electron build can encode every profile.

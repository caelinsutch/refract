...
### Selection rendering checks

New native-canvas pixel tests cover 1× and 2× preview density, visible handle interiors, untouched pixels away from the handles, expiration at the mask end timestamp, and removal of editor adornments by a fresh shared-compositor render. A separate partially cropped rectangle check confirms the visible outline renders while every pixel outside the recording bounds remains unchanged. The first interior-pixel assertion sampled the antialiased border; it was corrected to sample inside the white handle fill. All 103 tests pass, and the strengthened cropped-outline test passes separately. These are compositor checks, not evidence of live mouse interaction or encoded-export parity.

...
## Remembering the controller position
...
## Window source filtering and permission refresh

Native source discovery now asks ScreenCaptureKit to exclude desktop windows, then retains normal-layer windows owned by regular applications. It excludes Refract's parent process rather than all apps using the generic Electron bundle ID. Empty titles fall back to the application name. An empty picker explains how to open a window and refresh. Accessory/floating utility windows are excluded by this policy; exact reference filtering remains unverified. Apple's [shareable-content API](https://developer.apple.com/documentation/screencapturekit/scshareablecontent/getexcludingdesktopwindows(_:onscreenwindowsonly:completionhandler:)) documents the desktop exclusion.

Native and renderer production builds passed. Live packaged discovery retained ordinary application windows and removed the previously observed desktop, Dock, wallpaper, and software-cursor overlay entries. Opening Recording options now refreshes source permissions before showing its actions; a fresh renderer correctly reported the existing keyboard permission as enabled. Previously the panel could label an unqueried permission as needing enablement.

A live window recording with automatic zooms disabled completed at 12.036667 seconds and opened with an empty zoom lane; its capture configuration stored `automaticZooms: false`. The file contained no in-window cursor samples or key events, so this test establishes option propagation and successful finalization, not preservation of captured clicks. The existing synthetic finalization test supplies the latter evidence. The preference was restored to enabled after testing.

## Cursor geometry while moving a captured window

Window capture previously normalized cursor samples against the window rectangle discovered at recording start. It now updates that rectangle from each complete frame's `SCStreamFrameInfo.screenRect` metadata on the existing serial capture queue. Display and area recordings retain their selected capture bounds. This avoids a separate WindowServer query during cursor sampling. Apple documents [`screenRect`](https://developer.apple.com/documentation/screencapturekit/scstreamframeinfo/screenrect) as the onscreen location of captured content.

The native geometry verifier checks that moving the pointer and window together preserves the normalized location, including a display with negative coordinates, edge points, out-of-window positions, dictionary decoding, and invalid geometry. Native compilation and the verifier passed. The implementation retains the last valid bounds if metadata is absent or invalid. A live moving-window capture remains unverified; resizing can also introduce letterboxing in the fixed-size video surface, which still needs content-rectangle mapping. This change does not claim complete resize or multi-display capture parity.

## Cursor leaving the capture area

The native recorder now begins with a hidden cursor marker, marks in-bounds samples visible, and writes one hidden marker when the pointer leaves the capture rectangle. Previously, dropping out-of-bounds samples left the final pointer position visible indefinitely. Saved cursor events accept an optional boolean `visible`; older projects without that field retain their previous behavior.

The shared compositor gates pointer and click rendering on source-time presence, independently of idle hiding or cursor looping. Motion-blur samples exclude hidden portions, and spring checkpoints reset on re-entry. Looping targets the first visible retained position when capture begins outside the window. Hidden markers cannot generate automatic zooms.

Native and renderer builds passed; all 82 core tests passed. New checks cover exact enter/leave times, random seeks, re-entry spring/idle reset, legacy compatibility, saved-project validation, and pixel equality against a fully hidden cursor during edited source intervals with motion blur and looping enabled. A live recording that moves the physical cursor out of and back into the window remains pending. The compositor tests establish preview/export rendering behavior for these markers, not native event delivery.

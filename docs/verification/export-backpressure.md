...
## Auxiliary click-audio failure and cancellation

Export completion now joins FFmpeg and the click PCM pipeline through `completeEncoderInputs`. If either fails, the helper stops the encoder and waits for both promises to settle before allowing temporary-output cleanup. Encoder shutdown also destroys auxiliary pipes, and explicit cancellation waits for the joined completion before removing the temporary file. Previously, `Promise.all` could reject on an audio generation/pipe failure while FFmpeg was still alive.

Two real-child regression cases cover a synthesis failure and cancellation with a blocked auxiliary audio pipe. Both use an encoder stand-in that ignores SIGTERM: shutdown escalates to SIGKILL, the source and pipe are destroyed, the temporary file is removed, and a pre-existing destination remains unchanged. All 112 core tests and the production build pass. The actual FFmpeg MP4 click-audio verifier also uses the joined completion helper and passes, decoding clicks at 100/500/700 ms with silence between. This does not establish live UI cancellation or all mixed-audio export combinations.

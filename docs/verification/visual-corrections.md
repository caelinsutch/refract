...
## Native export menu and quick-export shortcuts — September 9
...
## Reference frame-rate choices — September 9

Added a shared source of supported output frame rates and used it in the editor, recorder quick settings, saved completion settings, main-process validation, and FFmpeg arguments. MP4 now supports 60/50/30/25/24/20/10; GIF supports 50/30/25/20/15/10. Unsupported format transitions choose the next lower compatible rate. The previously offered 24 fps GIF option was inconsistent with the reference and has been removed. See `docs/research/export-settings.md` for observed settings and remaining gaps.

Validation: production build and 183 core tests passed. All 39 format/size/frame-rate encoder combinations passed frame-count, duration, and GIF minimum-delay checks. The synthetic editor workflow passed with 20 fps GIF export, quick-export isolation, cancellation, and native menu availability. The six-second media check passed with 144 MP4 frames plus AAC audio and 120 GIF frames; decoded MP4 mean RGB error was 2.459/255. These tests establish Refract behavior, not encoded-pixel parity against the activation-gated reference.

## Editor layout proportions — September 9

Removed the extra timeline toolbar and moved cutting and timeline zoom into the playback row. The installed reference places these controls in ControlsBar rather than above the tracks. Matched the observed timeline dimensions: 20px horizontal margins, 25px ruler space, 48px tracks, 12px track gaps, 15px bottom space, and 4px holder padding. The default two-track section is now 152px rather than 205px, returning 53px to the preview/inspector. Playback speed and preview settings use compact 32px controls to prevent crowding at the minimum editor width. Existing 340px inspector width and 16px/14px body padding already match the reference declarations.

Validation: production build passed. The synthetic editor layout fixture passed at 1000×660, 1320×880, and 1600×1000 in both system themes, checking geometry and playback-control separation. Dark and light captures were visually inspected. The existing editor hover fixture passed, including timeline/trim isolation and playback menus. The installed reference editor still returns a white screenshot through the UI tool; these corrections use its inspected layout declarations and accessibility structure, not a completed screenshot pixel comparison. Full visual parity remains unverified.

## Timeline fit and horizontal scrolling — September 9
...
## Scissors split tool — September 9
...
## Packaged editor native workflow — September 9

Tested release/native-view through the macOS UI using the synthetic six-second source from work/editor-shortcuts. Imported it through the real file chooser with Go to Folder; the loaded editor exposed the full inspector, timeline, and enabled export controls. Command-Shift-Return hid the sidebar/timeline exactly once and a second invocation restored both, confirming native accelerator dispatch did not double-toggle the renderer. Command-right reached 0:06.00, Command-left returned to the start, and Right advanced the displayed position to 0:00.01. The live editor screenshot was inspected after these interactions.

This supersedes the earlier inability to complete a packaged import. The loaded native menu/shortcut path is now verified for this synthetic project. The Screen Studio editor still returns a white screenshot through the UI tool, so side-by-side pixel parity remains unverified. No desktop recording was started.

## Audio scrubbing control — September 9

Replaced the extra toolbar loop button with the reference's audio-scrubbing toggle; looping remains in the native View menu. Paused seeks audition separate source/microphone audio players without moving the video clock. The inspected reference uses 150ms samples at 75% volume. Refract applies clip/source mute and volume, source-time mapping, and caps a sample at the current clip's source end. The preference persists; disabling, entering playback/dialogs, or replacing media stops the players and cleans up resources.

Validation: production build and the synthetic audio-scrubber fixture passed. Real media decoding is used, with play calls captured instead of sent to speakers; the fixture verifies seek alignment, 0.75 gain, timed pause, mute suppression, disabled silence, and resource release. This does not establish audible quality or end-to-end microphone scrubbing parity; those remain to verify. Layout and native View-menu integration checks also passed.

## Decoded source and microphone scrub verification — September 9

Added a real-media hook harness with separate 440Hz source and 880Hz microphone WAVs. It runs actual HTMLAudioElement playback and inspects each stream through an analyser while Electron mutes the test window's output. The zero-gain analysis path initially returned silence; using an active graph with muted window output exposed the decoded signals without speaker playback.

Passed: a 500ms playback seek in a 2× clip trimmed to source 1000–3000ms starts both samples at source 2000ms. Source/microphone gain was 0.375/0.3 after clip and scrub attenuation. Spectral peaks were 430.66/882.86Hz (within FFT-bin tolerance). Samples stopped after the audition timer, at a near-end clip boundary, on playback suspension, and when disabled; disabling released media URLs. This establishes decoded source/microphone audition behavior for the synthetic tracks, while subjective audible quality and exact Screen Studio audio parity remain unverified.

## Output-height export sizing — September 9
